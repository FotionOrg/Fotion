    ...
    fetch(url, {
      ...
      .then(async (res) => {
        ...
        setSearchedDatabases(
          parsed.filter((db) => !projects.some((p) => p.sourceType === "NOTION" && p.databaseId === db.id)),
        )
      })
      .catch((err) => {
        console.error(err)
                  ...
                  <Command className="w-full" shouldFilter={false}>
                    ...
                    <CommandList className="bg-white border rounded shadow overflow-auto min-h-0 p-0">
                      {searchedDatabases.length > 0 &&
                        searchedDatabases
                          .filter((db) => db.id !== selectedDatabase?.id)
                          .map((database) => (
                            <CommandItem
                              key={database.id}
                              value={database.name}
          ...
          {selectedDatabase && (
            <div className="flex flex-col gap-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Estimated Duration(in minutes) Property</FormLabel>
                    <FormControl>
                      <Select
                        value={field.value}
                        onValueChange={(value) => {
                          ...
                          if (value === field.value) {
                            field.onChange("")
                            setSelectedPropertyIds(selectedPropertyIds.filter((id) => id !== value))
                          } else {
                            field.onChange(value)
                            setSelectedPropertyIds([...selectedPropertyIds.filter((id) => id !== field.value), value])
                          }
                        }}
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          {selectedDatabase?.properties.map((property) => (
                            <SelectItem
                              key={property.id}
                              value={property.id}
                              disabled={selectedPropertyIds.includes(property.id)}
                            >
                              {property.name}
                            </SelectItem>
                          ))}
